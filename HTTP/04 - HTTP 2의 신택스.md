...
## server-sent events
```
server-sent events 는 청크 형식의 조금씩 전송한다는 특징을 응용해, 서버에서 임의의 시점에 클라이언트에 이벤트를 통지할 수 있는 기능을 실현했다.
코멧의 롱 폴링과 청크 응답을 조합해, 한 번의 요청에 대해 서버에서 여러 이벤트 전송을 제공한다.

MIME 타입은 test/event-stream 이다.
```
...
통신이 확립되면 서버/클라이언트 사이에서 일대일 통신을 수행하고, 프레임 단위로 송수신 하지만
상대방이 정해져 있으므로 전송할 곳에 대한 정보는 갖지 않는다.
```

### 웹 소켓은 stateful
```
웹소켓이 HTTP 기반 프로토콜과 다른 점은 'stateful 통신'이라는 점이다.

웹소켓을 사용할 때 서버는 메모리에 데이터를 가진 상태로 통신하는 케이스가 많을 것이다.

문자 채팅과 같은 유스케이스에선 단일 서버에 모든 브라우저가 접속할 뿐 아니라,
멤캐시트나 레디스를 중계해서 부하를 분산시킬 수도 있다.

그러나 여러 사람이 동시에 실시간으로 플레이하는 게임처럼 부하 분산에 따른 지연을
허용할 수 없는 경우도 있다.
그러한 경우에 예를 들어 채팅이라면 '대화방' 단위로 커넥션을 온 메모리로 관리한다.
이런 경우 일단 접속이 끊어졌을때 재접속은 이전과 같은 서버로 연결할 필요가 있어,
단순한 HTTP 기반 로드 밸런서를 사용할 수 없다.
웹소켓 운용 시 클라이언트에서 서버를 지정해 재접속할 수 있도록 로드밸런서를 사용하지 않고 운용하는 사례도 있다.
```
...
5. 서버가 받아서 처리하면 클라이언트의 소켓 인스턴스는 송신 기능, 수신 기능이 활성화된다.

접속 후 소켓에 대해 클라이언트에서 하는 조작은 세 가지이다.
- send([데이터]): 메서드로 데이터를 서버로 송신
- onmessage: 이벤트 핸들러로 서버에 보내진 데이터를 수신
- close([코드[, 이유]]): 소켓 닫기

데이터로는 문자열, Blob, ArrayBuffer 등을 쓸 수 있다.
```
...
HTTP 웹 푸시는 기능의 특성상 브라우저가 그 시점에서 시작되지 않았거나 오프라인이라도
